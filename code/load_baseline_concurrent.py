...
RESULTS_FILE = os.path.join(RESULTS_DIR, "baseline_concurrent.csv")


# Identical physics to the sequential variant. This isolates one question:
# is the bottleneck fixable with plain concurrency on ONE machine (no cluster),
# or does it genuinely need a multi-node engine?
...
if __name__ == "__main__":
    print(f"Load test: baseline-concurrent ({args.workers} threads), {args.batches} batches, uncapped.")
            ...
            if submitted < args.batches:
                ...
                for tp, messages in msg_pack.items():
                    for message in messages:
                        ...
                        fut = pool.submit(run_task, work_order)
